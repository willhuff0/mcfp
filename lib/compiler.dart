...
class Compiler implements ast_expr.Visitor<CompiledExpr>, ast_stmt.Visitor<void> {
  ...
  Compiler({required String rootEnvName}) : _globalEnv = Environment(null, rootEnvName) {
    ...
    env.defineFunc('time', TimeCallable());
  }
  ...
  CompiledExpr visitCallExpr(ast_expr.Call expr) {
    ...
    if (func != null) {
      if (expr.arguments.length != func.arity()) {
        throw Exception('Expected ${func.arity()} arguments but got ${expr.arguments.length}.');
      }
...
class TimeCallable extends Callable {
  @override
  int arity() {
    return 0;
  }
